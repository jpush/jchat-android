...
public class VideoFileFragment extends BaseFragment {
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<VideoFileFragment> mFragment;

        public MyHandler(VideoFileFragment fragment) {
            mFragment = new WeakReference<VideoFileFragment>(fragment);
        }

        @Override
        public void handleMessage(android.os.Message msg) {
            super.handleMessage(msg);
            VideoFileFragment fragment = mFragment.get();
            if (fragment != null) {
                switch (msg.what) {
                    case SCAN_OK:
                        //关闭进度条
                        fragment.mAdapter = new VideoFileAdapter(fragment, fragment.mDocuments);
