...
public class DocumentFileFragment extends BaseFragment {
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<DocumentFileFragment> mFragment;

        public MyHandler(DocumentFileFragment fragment) {
            mFragment = new WeakReference<DocumentFileFragment>(fragment);
        }

        @Override
        public void handleMessage(android.os.Message msg) {
            super.handleMessage(msg);
            DocumentFileFragment fragment = mFragment.get();
            if (fragment != null) {
                switch (msg.what) {
                    case SCAN_OK:
                        //关闭进度条
                        fragment.mAdapter = new DocumentFileAdapter(fragment, fragment.mDocuments);
