...
public class ContactsFragment extends BaseFragment {
    ...
    public void onResume() {
        ...
        //为搜索群组做准备
        JChatDemoApplication.mGroupInfoList.clear();
                            ...
                            for (final Long groupID : groupIDList) {
                                JMessageClient.getGroupInfo(groupID, new GetGroupInfoCallback() {
                                    ...
                                    public void gotResult(int responseCode, String responseMessage, GroupInfo groupInfo) {
                                        if (responseCode == 0) {
                                            JChatDemoApplication.mGroupInfoList.add(groupInfo);
                                        }
                                    }
                                });
                            }
        ...
        //为搜索好友做准备
        if (JChatDemoApplication.mFriendInfoList != null)
            JChatDemoApplication.mFriendInfoList.clear();
        ContactManager.getFriendList(new GetUserInfoListCallback() {
            @Override
            public void gotResult(int i, String s, List<UserInfo> list) {
                if (i == 0) {
                    JChatDemoApplication.mFriendInfoList = list;
                }
            }
        });
    }
    ...
    public void onEvent(ContactNotifyEvent event) {
        ...
        if (event.getType() == ContactNotifyEvent.Type.invite_accepted) {
            ...
            JChatDemoApplication.forAddFriend.remove(username);
            FriendRecommendEntry entry = FriendRecommendEntry.getEntry(user, username, appKey);
            ...
            if (JChatDemoApplication.forAddFriend.size() > 0) {
                for (String forAdd : JChatDemoApplication.forAddFriend) {
                    if (forAdd.equals(username)) {
                        return;
                    } else {
                        JChatDemoApplication.forAddFriend.add(username);
                    }
                }
            } else {
                JChatDemoApplication.forAddFriend.add(username);
